...
### 速度排行頁（SpeedPage）
- [x] 路由：`/speed`，Nav 加入「速度排行 / Speed」連結
- [x] 資料：只包含 Champions 寶可夢（272 隻）+ Mega 型態
- [x] 速度計算：Lv50・31 IV・0 EV・無性格加成（base + 20）
- [x] 滾輪式鼓面效果：中央列正常大小，上下漸進縮小 + 透明，最邊緣明顯變淺
- [x] 同速複數寶可夢橫排顯示（wrap 換行），置中排列
- [x] 左側速度標題：Lv50 大字 + base 小字，固定寬度右對齊
- [x] 藍色帶 snap highlight：距中心最近的列自動套用圓角藍色底
- [x] CSS mask-image 遮蔽上下邊緣，SPACER = 容器高 40% 與 mask 透明區完全對齊
- [x] 自動捲動至最高速（首次載入）
- [x] 搜尋框：右上角，支援中英文名稱搜尋，定位並高亮 match 的寶可夢
- [x] 搜尋高亮：白芯 + 檸檬黃漸層光暈，無框圓圈樣式
- [x] Enter 鍵循環跳轉多個 match，顯示 `n / total` 計數
- [x] Mega 型態支援（名稱顯示 + Mega 符號 badge）
- [x] 載入進度提示（loadedCount / total）
