    ...
    <div className="relative shrink-0">
      ...
      {open && (
        <>
          ...
          <div className="absolute left-0 top-full mt-1.5 bg-white border border-clay-border rounded-[16px] shadow-clay-md overflow-hidden z-50 min-w-[140px]">
            {BATTLE_ITEMS.map((item, i) => {
              ...
              if (!item.active) {
                return (
                  <div key={i} className="flex items-center justify-between px-4 py-2.5 opacity-40 cursor-not-allowed">
                    <span className="text-sm text-clay-silver">{label}</span>
                    <span className="text-[10px] text-clay-silver ml-2">soon</span>
                  </div>
                );
              }
              ...
              return (
                <Link
                  ...
                  className={`block px-4 py-2.5 text-sm font-semibold transition-colors hover:bg-clay-oat ${
                    isActive ? 'text-clay-blue bg-clay-blue-light' : 'text-clay-charcoal'
                  }`}
                >
                  {label}
                </Link>
