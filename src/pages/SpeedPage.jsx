    ...
    <div className="min-h-[calc(100vh-3.5rem)] bg-clay-cream">
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 xl:px-10 py-6">
        ...
        <div className="flex items-start justify-between gap-4 mb-4">
          <div>
            <h1 className="text-2xl font-black text-clay-charcoal mb-0.5">
          ...
          <div className="flex items-center gap-2 mt-1 shrink-0">
            <div className="relative">
              <input
                ...
                className="pl-8 pr-7 py-1.5 text-sm rounded-[16px] border border-clay-border bg-white shadow-clay focus:outline-none focus:ring-2 focus:ring-clay-blue/30 focus:border-transparent w-44"
              />
        ...
        {!isFullyLoaded && (
          <p className="text-xs text-clay-blue mb-4 tabular-nums">
            {zh ? `載入中 ${loadedCount} / ${total}…` : `Loading ${loadedCount} / ${total}…`}
          </p>
        )}

        {/* Drum wrapper */}
        <div className="rounded-[16px] overflow-hidden border border-clay-border shadow-clay">
            ...
            {rows.map((row, i) => (
              ...
              >
                <div className="shrink-0 w-16 text-right">
                  <p className="text-2xl font-black text-clay-charcoal tabular-nums leading-none">{row.lv50}</p>
                  <p className="text-[10px] text-clay-silver tabular-nums mt-0.5">base {row.base}</p>
                </div>
                <div className="shrink-0 w-px self-stretch bg-clay-border my-1" />
                  ...
                  {row.pokemon.map(p => {
                    ...
                    return (
                      <div key={p.apiName} className="flex flex-col items-center gap-1" style={{ minWidth: '80px' }}>
                        ...
                        >
                          {p.sprite
                            ? <img
                              ...
                              />
                            : <div className="w-20 h-20 bg-clay-oat rounded-full" />}
                          {p.isMega && (
                        ...
                        <p className="text-xs text-clay-charcoal text-center whitespace-nowrap leading-tight">{line1}</p>
                        {line2 && <p className="text-[10px] text-clay-silver text-center whitespace-nowrap leading-tight">{line2}</p>}
                      </div>
                    );
                  })}
                </div>
